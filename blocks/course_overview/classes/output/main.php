...
class main implements renderable, templatable {
     ...
     *
     * @var string String matching the grouping constants defined in course_overview/lib.php
     */
    private $grouping;
     ...
     *
     * @var string String matching the sort constants defined in course_overview/lib.php
     */
    private $sort;
     ...
     *
     * @var string String matching the view/display constants defined in course_overview/lib.php
     */
    private $view;
     ...
     *
     * @var string String matching the paging constants defined in course_overview/lib.php
     */
    private $paging;
    ...
    /**
     * Store the configuration values for the course_overview block.
     *
     * @var array Array of available layouts matching view/display constants defined in course_overview/lib.php
     */
    private $layouts;
    ...
    public function __construct($grouping, $sort, $view, $paging, $customfieldvalue = null) {
        ...
        if ($sort) {
            ...
            $this->sort = BLOCK_COURSE_OVERVIEW_SORTING_SHORTNAME;
        } else {
            $this->sort = BLOCK_COURSE_OVERVIEW_SORTING_TITLE;
        }
        // In case sorting remembered is shortname and display extended course names not checked,
        // we should revert sorting to title.
        if (!$CFG->courselistshortnames && $sort == BLOCK_COURSE_OVERVIEW_SORTING_SHORTNAME) {
            $this->sort = BLOCK_COURSE_OVERVIEW_SORTING_TITLE;
        }

        // Check and remember the given view.
        $this->view = $view ? $view : BLOCK_COURSE_OVERVIEW_VIEW_CARD;

        // Check and remember the given page size, `null` indicates no page size set
        // while a `0` indicates a paging size of `All`.
        if (!is_null($paging) && $paging == BLOCK_COURSE_OVERVIEW_PAGING_ALL) {
            $this->paging = BLOCK_COURSE_OVERVIEW_PAGING_ALL;
        } else {
            $this->paging = $paging ? $paging : BLOCK_COURSE_OVERVIEW_PAGING_12;
        }

        // Check and remember if the course categories should be shown or not.
        if (!$config->displaycategories) {
            $this->displaycategories = BLOCK_COURSE_OVERVIEW_DISPLAY_CATEGORIES_OFF;
        } else {
            $this->displaycategories = BLOCK_COURSE_OVERVIEW_DISPLAY_CATEGORIES_ON;
        }
    ...
    private function get_fallback_grouping($config) {
        if ($config->displaygroupingall == true) {
            return BLOCK_COURSE_OVERVIEW_GROUPING_ALL;
        }
        if ($config->displaygroupingallincludinghidden == true) {
            return BLOCK_COURSE_OVERVIEW_GROUPING_ALLINCLUDINGHIDDEN;
        }
        if ($config->displaygroupinginprogress == true) {
            return BLOCK_COURSE_OVERVIEW_GROUPING_INPROGRESS;
        }
        if ($config->displaygroupingfuture == true) {
            return BLOCK_COURSE_OVERVIEW_GROUPING_FUTURE;
        }
        if ($config->displaygroupingpast == true) {
            return BLOCK_COURSE_OVERVIEW_GROUPING_PAST;
        }
        if ($config->displaygroupingfavourites == true) {
            return BLOCK_COURSE_OVERVIEW_GROUPING_FAVOURITES;
        }
        if ($config->displaygroupinghidden == true) {
            return BLOCK_COURSE_OVERVIEW_GROUPING_HIDDEN;
        }
        if ($config->displaygroupingcustomfield == true) {
            return BLOCK_COURSE_OVERVIEW_GROUPING_CUSTOMFIELD;
        }
    ...
    public function set_available_layouts() {
        ...
        if ($config = get_config('block_course_overview', 'layouts')) {
            $this->layouts = explode(',', $config);
        } else {
            $this->layouts = array(BLOCK_COURSE_OVERVIEW_VIEW_CARD);
        }
    }
    ...
    public function format_layout_for_export($layoutname) {
        ...
        $layout->name = get_string($layoutname, 'block_course_overview');
        $layout->active = $this->view == $layoutname ? true : false;
        $layout->arialabel = get_string('aria:' . $layoutname, 'block_course_overview');
    ...
    public function get_customfield_values_for_export() {
        ...
        $customfieldactive = ($this->grouping === BLOCK_COURSE_OVERVIEW_GROUPING_CUSTOMFIELD);
        $ret = [];
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $nocoursesurl = $output->image_url('courses', 'block_course_overview')->out();
        ...
        $selectedcustomfield = '';
        if ($this->grouping == BLOCK_COURSE_OVERVIEW_GROUPING_CUSTOMFIELD) {
            ...
            if (!$selectedcustomfield) {
                $this->grouping = $this->get_fallback_grouping(get_config('block_course_overview'));
                if ($this->grouping == BLOCK_COURSE_OVERVIEW_GROUPING_CUSTOMFIELD) {
        ...
        $sort = '';
        if ($this->sort == BLOCK_COURSE_OVERVIEW_SORTING_SHORTNAME) {
            $sort = 'shortname';
        } else {
            $sort = $this->sort == BLOCK_COURSE_OVERVIEW_SORTING_TITLE ? 'fullname' : 'ul.timeaccess desc';
        }
    ...
    public function export_for_zero_state_template(renderer_base $output) {
        ...
        $nocoursesimg = $output->image_url('courses', 'block_course_overview');
            ...
            if (!$totalcourses && ($category = \core_course_category::get_nearest_editable_subcategory($coursecat, ['create']))) {
                ...
                if ($quickstarturl) {
                    $quickstartbutton = new \single_button(
                        ...
                        get_string('viewquickstart', 'block_course_overview'),
                    );
                ...
                $createbutton = new \single_button(
                    ...
                    get_string('createcourse', 'block_course_overview'),
                    'post',
                    \single_button::BUTTON_PRIMARY
                );
    ...
    private function generate_zero_state_data(\moodle_url $imageurl, array $buttons, array $strings) {
        ...
        $docparams = [
            ...
            'quicktitle' => get_string('viewquickstart', 'block_course_overview'),
            'quicktarget' => '_blank',
            'dochref' => $dochref->out(),
        ...
        return [
            ...
            'title' => ($strings['title']) ? get_string($strings['title'], 'block_course_overview') : '',
            'intro' => ($strings['intro']) ? get_string($strings['intro'], 'block_course_overview', $docparams) : '',
            'buttons' => $buttons,
        ];
    }
}
