...
class block_loms_course_enrl extends block_base {
    public function init() {
        $this->title = get_string('pluginname', 'block_loms_course_enrl');
    }
    ...
    public function get_content() {
        ...
        $lomsCurrentUserId = $USER->id;

        $context = context_course::instance($COURSE->id, MUST_EXIST);

        $lomsEnroled = get_string('course_enrolled', 'theme_loms');
        $lomsEnroledText = get_string('course_enrolled_text', 'theme_loms');

        if( function_exists('isguestuser')
          && !isguestuser()
          && isloggedin()
          && is_enrolled($context, $USER, '', true)
          && isset($lomsCourse->teachers[$lomsCurrentUserId])
          && ($lomsCurrentUserId == $lomsCourse->teachers[$lomsCurrentUserId]->userId)
          ){
            $lomsEnroled = get_string('course_enrolled_teacher', 'theme_loms');
            $lomsEnroledText = get_string('course_enrolled_teacher_text', 'theme_loms');
        }
        ...
        <div class="course-details-sidebar">
            ...
            <div class="sidebar-futcher">';
                if($this->content->price){
                    ...
                    }elseif($lomsCourse->course_price) {
                        $text .='
                        <div class="sidebar-title d-flex justify-content-between">
                            <h2> ' . get_config('theme_loms', 'site_currency') .'' . $lomsCourse->course_price .'</h2> 
                ...
                if(!$lomsCourse->course_is_enrolled):
                    $text .='
                    <div class="cart-wish d-flex justify-content-between">
                        <a href="'. $lomsCourse->enrolmentLink .'" class="default-btn">
                        '. get_string('course_enrolment', 'theme_loms') .'
                        </a>
                    </div>';
                endif;
                $text .='
