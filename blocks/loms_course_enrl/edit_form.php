...
class block_loms_course_enrl_edit_form extends block_edit_form {
    ...
    protected function specific_definition($mform) {
        ...
        $mform->addElement('text', 'config_video_link', get_string('config_video', 'theme_loms'));
        ...
        $mform->addElement('select', 'config_items', get_string('config_items', 'block_loms_course_enrl'), $itemsrange);

        $mform->addElement('text', 'config_price', get_string('config_price', 'theme_loms'));
        ...
        for($i = 1; $i <= $items; $i++) {
            ...
            $mform->addElement('text', 'config_item_title' . $i, get_string('config_item_title', 'block_loms_course_enrl', $i));
            $mform->setDefault('config_item_title' .$i , 'Course Level');
            $mform->setType('config_item_title' . $i, PARAM_TEXT);

            $mform->addElement('text', 'config_item_value' . $i, get_string('config_item_value', 'block_loms_course_enrl', $i));
            $mform->setDefault('config_item_value' .$i , 'Course Level');
            $mform->setType('config_item_value' . $i, PARAM_TEXT);

            $select = $mform->addElement('select', 'config_item_icon' . $i, get_string('config_item_icon', 'block_loms_course_enrl'), $lomsFontList, array('class'=>'loms_icon_class'));
