 ...
 * Blog renderer
 */
class theme_loms_core_blog_renderer  extends core_blog_renderer  {
    ...
    public function render_blog_entry(blog_entry $entry) {
        ...
        if ( strpos($actual_link, '/index.php?entryid') == false ):
            ...
            $o = $this->output->container_start($mainclass, 'b' . $entry->id);
                $o .= $this->output->container_start('single-blog before-none', '' . $entry->id);

                    $o .= $this->output->container_start('d-block blog-img');
                        // Attachments.
            ...
            $o = $this->output->container_start($mainclass, 'b' . $entry->id);
                ...
                if (!empty($entry->renderable->comment)) {
                    ...
                    $loms_comments = new comment($cmt);
                    $loms_comments->set_view_permission(true);
                    $loms_comments->set_fullwidth();

                    $o .= $loms_comments->output(true);
                }
            $o .= $this->output->container_end();
